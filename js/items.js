...
function funcMemory(elem) {
    ...
    if (colorArr.memoryArr.indexOf(elem.name)<0) {
        colorArr.memoryArr.push(elem.name)

    }
    else if(colorArr.memoryArr.indexOf(elem.name)>-1 ) {
        colorArr.memoryArr.splice(colorArr.memoryArr.indexOf(elem.name), 1)
        renderProducts(arrFiltred);
    }
...
function funcOs(osType){
    ...
    if (' ') {
        arrFiltred = items.filter((osSome) => {
            for (let memory of colorArr.OSarr) {
                 if(osSome.os == memory){
                    console.log(osSome); 
                    return osSome;
    ...
    renderProducts(arrFiltred);
    console.log(arrFiltred, colorArr.OSarr);
}
...
function funcDisplay(display){
    ...
    if (' ') {
        arrFiltred = items.filter((disSome) => {
            for (let disp of colorArr.displayArr) {
                let diag =(disSome.size.height**2+ disSome.size.width**2)**0.5
                let dispValue = disp.split(',');
                
                if(+diag > +dispValue[0] && diag < +dispValue[1]){
                    console.log(diag,dispValue); 
                    return disSome;
    ...
    renderProducts(arrFiltred);
        console.log(arrFiltred, colorArr.displayArr);
}
renderProducts(arrFiltred);
console.log(colorArr.memoryArr,colorArr,arrFiltred);
